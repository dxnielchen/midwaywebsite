...
export default function AdminParty() {
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/party?id=${id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
    fetchPhotos();
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-2xl font-bold mb-1">Party Photos</h1>
          <p className="text-white/40 text-sm">These rotate on the Book a Party page.</p>
        </div>
      ...
      {photos.length === 0 ? (
        <div className="admin-card text-center py-16">
          ...
          <p className="text-white/30">No photos yet. Upload your first party image.</p>
        </div>
      ) : (
